#' alignbygene function
#'
#' This function plots the gene arrow map with directionality + aligns the genes as per the user selection
#' @export
#'

alignbygene <- function(colpal="c25", geneName="genD", arrowhead_height="5", arrowhead_width="3", element_line="gray"){
...
if (colpal=="c25") {
  ...
  p_aligned1 <- ggplot(data(), aes(xmin = start, xmax = end, y = molecule,fill = gene, label = gene,  forward = strand)) +
  geom_gene_arrow(arrowhead_height = unit(arrowhead_height, "mm"),arrowhead_width = unit(arrowhead_width, "mm")) +
  geom_gene_label(align = "centre", grow = TRUE,reflow = TRUE) +
